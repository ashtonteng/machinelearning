...
def freq_viagra_feature(text, freq):
    return float(freq['viagra']) + float(freq['viag']) + float(freq['vlagra']) + float(freq['cialis'])
...
def freq_sex_feature(text, freq):
    return float(freq['sex']) + float(freq['sexual']) + float(freq['sexy'])
...
def freq_bcc_feature(text, freq):
    return float(freq['bcc'])

def common_spam(text, freq, common_spam):
    freq_list = []
    for spam_word in common_spam:
        freq_list.append(float(freq[spam_word]))
    return freq_list

def common_ham(text, freq, common_ham):
    freq_list = []
    for ham_word in common_ham:
        freq_list.append(float(freq[ham_word]))
    return freq_list
...
def generate_feature_vector(text, freq):
    ...
    feature.append(freq_bcc_feature(text, freq))
    feature.append(freq_erection_feature(text, freq))
    common_spam_file = open("common_spam.txt", "r")
    common_spam_text = common_spam_file.read()
    common_spam_words = common_spam_text.split(" ")
    feature += common_spam(text, freq, common_spam_words)
    common_ham_file = open("common_ham.txt", "r")
    common_ham_text = common_ham_file.read()
    common_ham_words = common_ham_text.split(" ")
    feature += common_ham(text, freq, common_ham_words)
    #print(feature)
    return feature
